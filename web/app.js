...
const PDFViewerApplication = {
  ...
  bindEvents() {
    ...
    eventBus._on(
      ...
      async evt => {
        console.log(
          `[app.js] 收到inkstatechanged事件（方案F：增量更新），油墨数量: ${evt.inks?.length}`
        );
        ...
        // 方案F：增量更新（不重新加载文档）
        if (!this.pdfDocument) {
          console.warn(`[app.js] pdfDocument不存在，无法进行增量更新`);
          return;
        }

        try {
          // 步骤1：同步配置到Worker线程（不重新加载文档）
          if (this.pdfDocument.updateColorFilterConfig) {
            ...
            console.warn(
              `[app.js] updateColorFilterConfig方法不存在，回退到完全重载`
            );
            throw new Error("updateColorFilterConfig方法不存在");
          }

          // 步骤2：清除受影响页面的缓存
          console.log(`[app.js] 开始清除缓存`);
          const visiblePages = this.pdfViewer._getVisiblePages();
          ...
          if (this.url && this.pdfLoadingTask) {
            console.log(`[app.js] 回退方案A：重新加载文档`);
            ...
            try {
              ...
              if (
                this.pdfDocument &&
                this.pdfDocument.updateColorFilterConfig
              ) {
                await this.pdfDocument.updateColorFilterConfig(config);
              }
              ...
              console.error(`[app.js] 回退方案也失败:`, fallbackError);
            }
          }
        }
      },
